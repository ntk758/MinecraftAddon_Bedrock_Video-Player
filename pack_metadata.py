...
RELEASE_NOTES = [
    ...
    "高速化: v3バイナリフォーマットを導入。全フレームを1本のバイナリに連結+コンパクトインデックスにより、ワールド読み込み時間を桁違いに短縮",
    "画質・速度革命: v4.0.0 Phase 6。OkLab知覚色空間への完全移行による色再現性の劇的向上、シーン適応型の自動圧縮制御(RDO/ME)、NumPyベクトル化によるエンコード高速化、予測型GOPプリフェッチとスマートティック予算による超安定再生を実現",
    "高速化: v4 GOP-Chunked遅延デコード。200フレーム単位の独立チャンク分割+LRUキャッシュにより、高解像度動画の読み込み速度を劇的に改善",
]
