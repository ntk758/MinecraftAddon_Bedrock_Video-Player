...
### ✅ A6. 局所的SSIMを用いた知覚的RDO
- **理由**: 単純なMSE/SADによる圧縮では、文字やキャラクターのエッジが潰れやすい。人間の目は構造的な歪みに敏感であるため。
- **効果**: 局所的なSSIMを評価関数に組み込むことで、重要なディテールを保持しつつ、背景などの冗長な部分のビットレートを下げる「知覚的RDO」を実現。

### ✅ A7. 動的評価式に基づくシーン適応型GOP
- **理由**: 固定長のGOP(例: 30フレーム)では、静かなシーンで無駄なIフレームが挿入され、激しいシーンではPフレームの破綻が起きる。
- **効果**: `0.5*SAD + 0.3*Hist + 0.2*Edge` を複合的に評価し、シーンチェンジを検知。動的にGOPサイズを可変させることで圧縮率と品質のトレードオフを最適化。

### ✅ A8. LPIPS vs DeltaE の統合ベンチマーク評価 (benchmark/導入)
- **理由**: マイクラブロックによる特殊な色表現では、$\Delta E$ だけで人間の体感画質を測るのが難しい。
- **効果**: SSIM, PSNR, LPIPS, $\Delta E_{2000}$ を総合的に測定できるモジュラー設計のベンチマークフレームワーク (`run.py --deep`) を `benchmark/` ディレクトリに構築し、多角的な評価を可能にした。

### ✅ A9. オブジェクト指向 VideoPlayer クラスへのリファクタリング
- **理由**: 従来のシングルトン的な関数設計では、ワールド内で1つの画面しか再生できなかった。
- **効果**: 状態をインスタンスにカプセル化する `VideoPlayer` クラスを実装し、マルチスクリーン同時ストリーミング再生に成功。
